...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed lg:static inset-y-0 left-0 z-50 w-64 border-r-2 border-terminal-border 
          flex flex-col bg-black/95 lg:bg-black/40 shrink-0 transform transition-transform duration-300
      ...
      >
        <div className="bg-terminal-border/40 px-4 py-2 flex justify-between items-center text-[11px] uppercase font-bold text-neon-purple border-b-2 border-terminal-border crt-glow-purple">
          ...
          <span className="material-symbols-outlined text-xs cursor-pointer hidden lg:block">close_fullscreen</span>
        </div>
        ...
        <div className="p-4 flex-1 overflow-y-auto custom-scrollbar">
          <nav className="space-y-2 text-[12px]">
            <button
              ...
              className={`w-full flex items-center justify-between py-2 px-1 rounded transition-colors ${viewMode === ViewMode.LIBRARY
            ...
            <button
              onClick={() => { onSelectPlaylist(null); onClose(); }}
              className={`w-full flex items-center justify-between py-2 px-1 rounded transition-colors ${viewMode === ViewMode.PLAYLIST && !selectedPlaylistId
                  ? 'text-white bg-neon-purple/20 border-l-2 border-neon-purple'
                  : 'text-cyber-cyan hover:text-white'
                }`}
            >
            ...
            {playlists.map((pl) => {
              ...
              return (
                <button
                  key={pl.id}
                  onClick={() => { onSelectPlaylist(pl); onClose(); }}
                  className={`w-full flex items-center justify-between py-2 px-1 rounded transition-colors ${isActive
                      ? 'text-white bg-neon-purple/20 border-l-2 border-neon-purple'
                      : 'text-cyber-cyan hover:text-white'
                    }`}
                >
        ...
        <div className="p-4 border-t-2 border-terminal-border bg-black space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              onClick={() => { onSelectUpload(); onClose(); }}
